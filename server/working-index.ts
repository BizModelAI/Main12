...
app.get("/api/health/detailed", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!prisma) {
      throw new Error("Database prisma client not available");
    }
...
async function setupApp() {
  try {
    ...
    // FINAL catch-all for /api/* routes: always return JSON 404 if no API route matched
    ...
    try {
      ...
      // Patch: Only use Vite middleware for non-API routes
