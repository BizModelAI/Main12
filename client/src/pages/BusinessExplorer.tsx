...
const BusinessExplorer: React.FC<BusinessExplorerProps> = ({
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      ...
      try {
        // Always fetch the latest paid quiz attempt for business model fit
          ...
          <div className="flex flex-col gap-1 items-end">
            ...
            {showFitBadge && fitScore !== undefined && fitScore > 0 && (
              <div className="text-right">
                <div className="text-2xl font-bold text-blue-600" data-testid={`fit-percentage-${model.id}`}> 
                  {Math.round(fitScore)}%
                </div>
                <div className="text-xs text-gray-500" data-testid={`fit-category-${model.id}`}> 
