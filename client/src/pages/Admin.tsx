...
const AdminPage: React.FC = () => {
  ...
  // Auto-populate refund amount when payment is selected
  useEffect(() => {
    if (selectedPayment) {
      setRefundForm(prev => ({
        ...prev,
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPayments(data.payments || data); // Handle both nested and direct array responses
      } else {
        const errorData = await response.json().catch(() => ({}));
        setError(`Failed to fetch payments: ${errorData.error || response.statusText}`);
        console.error("Failed to fetch payments:", response.status, errorData);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      setError(`Error fetching payments: ${errorMessage}`);
  ...
  const fetchRefunds = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setRefunds(data.refunds || data); // Handle both nested and direct array responses
      } else {
        const errorData = await response.json().catch(() => ({}));
  ...
  const processRefund = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      setError(`Error processing refund: ${errorMessage}`);
