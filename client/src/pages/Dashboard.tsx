  ...
  const quickActions = [
    {
      ...
      onClick: async () => {
        try {
          console.log("View Full Report clicked - fetching latest quiz data");
          
          // Fetch the latest quiz data from the API
          ...
          if (response.ok) {
            ...
            console.log("Latest quiz data fetched:", data);
            
            if (data.quizData) {
              // Store the quiz data in localStorage so the Results component can access it
              ...
              console.error("No quiz data found in API response");
              // Still navigate but show an error message
              navigate("/results?showFullReport=true", { replace: true });
            }
          } else {
            console.error("Failed to fetch latest quiz data:", response.status);
            // Still navigate but show an error message
            navigate("/results?showFullReport=true", { replace: true });
          }
        } catch (error) {
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <button
                    onClick={async () => {
                      try {
                        console.log("View Full Results clicked - fetching latest quiz data");
