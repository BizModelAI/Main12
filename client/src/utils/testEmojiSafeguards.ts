...
export function testEmojiSafeguards() {
  ...
  validEmojis.forEach(emoji => {
    ...
    console.log(`${emoji} is valid: ${isValid} ${isValid ? 'PASS' : 'FAIL'}`);
  });
  ...
  corruptedEmojis.forEach(({ emoji, businessId, expected }) => {
    ...
    console.log(`"${emoji}" → "${fixed}" (expected: "${expected}") ${success ? 'PASS' : 'FAIL'}`);
  });
  ...
  console.log(`Business model emoji fixed: "${testBusinessModel.emoji}" → "${safeguarded.emoji}" ${success ? 'PASS' : 'FAIL'}`);
  ...
  safeguardedArray.forEach((item, index) => {
    console.log(`${item.name}: "${testArray[index].emoji}" → "${item.emoji}" ${item.emoji === expectedEmojis[index] ? 'PASS' : 'FAIL'}`);
  });
  ...
  testMappings.forEach(businessId => {
    ...
    const isValid = isValidEmoji(emoji);
    console.log(`${businessId}: ${emoji} ${isValid ? 'PASS' : 'FAIL'}`);
  });
