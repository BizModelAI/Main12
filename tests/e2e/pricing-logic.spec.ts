...
test.describe('Pricing Logic Tests', () => {
  ...
  test('New user (temporary) sees $9.99 pricing after completing quiz', async ({ page }) => {
    ...
    if (await paywallElement.isVisible()) {
      ...
      // If no paywall, check if pricing is shown elsewhere
      const pricingElement = page.locator('text=$9.99, text=$4.99').first();
  ...
  test('Paid user (non-temporary) sees $4.99 pricing after completing quiz', async ({ page }) => {
    // Navigate to the quiz
    await page.goto('/');
  ...
  test('Temporary user sees $9.99 pricing even if authenticated', async ({ page }) => {
    // Navigate to the quiz
    await page.goto('/');
    ...
    // Simulate a temporary user by setting localStorage flags
    try {
      await page.evaluate(() => {
        localStorage.setItem('user', JSON.stringify({ id: 1, email: 'test@example.com', isTemporary: true }));
      });
    } catch (error) {
      console.log('localStorage access blocked, continuing...');
    }
    ...
    if (await paywallElement.isVisible()) {
      // Temporary user should see $9.99 even if they have an account
      await expect(page.locator('text=$9.99')).toBeVisible();
      await expect(page.locator('text=$4.99')).not.toBeVisible();
    } else {
      // If no paywall, check if pricing is shown elsewhere
